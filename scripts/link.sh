...
echo "[2/9] Linking editor configs..."
...
echo "[5/9] Linking Claude Code configs..."
mkdir -p "$HOME/.claude"
...
echo "[6/9] Copying KDE configs (no symlinks - KDE overwrites them)..."
...
echo "[7/9] Linking SSH config..."
mkdir -p "$HOME/.ssh"
link_file "$DOTFILES/.ssh/config" "$HOME/.ssh/config"

echo "[8/9] Linking XDG configs..."
...
echo "[9/9] Copying emulator configs (no symlinks - emulators overwrite them)..."
